...
from minicode.client import LLMClient
...
from minicode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from minicode.conversation import ThinkingBlock as ConvThinkingBlock
from minicode.memory.auto_memory import MemoryManager
from minicode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from minicode.hooks import HookContext, HookEngine, ToolRejectedError
from minicode.hooks.engine import HookNotification
from minicode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".minicode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".minicode" / "plans"
        ...
        while True:
            ...
            if not response.tool_calls:
                ...
                if self.file_history is not None:
                    summary = response.text[:60] + \
                        "..." if len(response.text) > 60 else response.text
                    self.file_history.make_snapshot(
                        len(conversation.history), summary)
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from minicode.permissions.rules import Rule, extract_content
                    content = extract_content(tc.tool_name, tc.arguments)
                    pattern = f"{content[:60]}*" if len(
                        content) > 60 else f"{content}*"
                    # 持久化规则写入本地文件
                    rule = Rule(tool_name=tc.tool_name,
    ...
    def _snapshot_for_recovery(
        ...
        if result.is_error or tc.tool_name != "ReadFile":
            return
        path = tc.arguments.get("file_path") if isinstance(
            tc.arguments, dict) else None
        if not path:
            return
        try:
