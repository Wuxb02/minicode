...
from minicode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from minicode.commands.completion import CompletionPopup
from minicode.commands.handlers import register_all_commands
from minicode.config import MCPServerConfig, ProviderConfig
from minicode.hooks import HookContext, HookEngine, load_hooks
from minicode.conversation import ConversationManager, Message
from minicode.mcp import ConnectResult, MCPManager
...
from minicode.agents.task_manager import TaskManager
from minicode.agents.trace import TraceManager
from minicode.agents.notification import inject_task_notifications
from minicode.commands.handlers.tasks import create_tasks_command
from minicode.skills.executor import SkillExecutor
from minicode.skills.loader import SkillLoader
from minicode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from minicode.cache import FileCache
from minicode.tools import ToolRegistry, create_default_registry
from minicode.tools.agent_tool import AgentTool
from minicode.tools.ask_user import AskUserEvent, AskUserTool
from minicode.tools.impl.tool_search import ToolSearchTool
from minicode.tools.load_skill import LoadSkill
from minicode.worktree.cleanup import start_stale_cleanup_task
from minicode.worktree.manager import WorktreeManager
from minicode.commands.handlers.worktree import create_worktree_command
from minicode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv",
              "__pycache__", ".minicode", "build", ".gradle"}


def scan_files_for_at(prefix: str, work_dir: str, limit: int = 10) -> list[str]:
    matches: list[str] = []
    base = os.path.join(work_dir, os.path.dirname(prefix)
                        ) if "/" in prefix else work_dir
    ...
    try:
        for entry in sorted(os.listdir(base)):
            ...
            if entry.lower().startswith(name_prefix):
                rel = os.path.join(os.path.dirname(prefix),
                                   entry) if "/" in prefix else entry
    ...
    def _replace(m: re.Match) -> str:
        ...
        try:
            content = open(full_path, encoding="utf-8",
                           errors="replace").read(MAX_AT_REF_BYTES)
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".minicode" / "history"
...
class minicodeApp(App):
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append(" · ".join(parts), style="color(242)")
        return t
    ...
    def on_mount(self) -> None:
        self.register_theme(_minicode_THEME)
        self.theme = "minicode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".minicode" / "permissions.yaml",
                project_rules_path=Path(work_dir) /
                ".minicode" / "permissions.yaml",
                local_rules_path=Path(work_dir) /
        ...
        from minicode.filehistory import FileHistory
        ...
        from minicode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from minicode.config import WorktreeConfig
        ...
        from minicode.tools.enter_worktree import EnterWorktreeTool
        from minicode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from minicode.teams.manager import TeamManager
        from minicode.tools.team_create import TeamCreateTool
        from minicode.tools.team_delete import TeamDeleteTool
        ...
        from minicode.commands.handlers.trace import create_trace_command
        trace_cmd = create_trace_command(
            self.trace_manager, self.agent.agent_id)
        self.command_registry.register_sync(trace_cmd)

        # --- 协调者模式初始化（工具已注册，激活推迟到 TeamCreate 时） ---
        from minicode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from minicode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from minicode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from minicode.plan_dialog import InlinePlanWidget, PlanChoice
        from minicode.prompts import build_plan_mode_exit_reminder
        ...
        if choice == PlanChoice.YOLO:
            ...
            exit_msg = build_plan_mode_exit_reminder(
                str(plan_path), plan_exists)
    ...
    def _tick_spinner(self) -> None:
        ...
        if self._spinner_label is not None:
            ...
            if self._spinner_idx % 5 == 0:
                try:
                    self.query_one(
                        "#chat-area", VerticalScroll).scroll_end(animate=False)
                except Exception:
                    pass
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from minicode.permission_dialog import InlinePermissionWidget
    ...
    def _update_mode_label(self) -> None:
        if self.agent:
            ...
            if perm == PermissionMode.DEFAULT:
                ...
                label.update(
                    f"[{color}]{display}[/{color}]  (shift+tab to cycle)")
        try:
            ...
            if self._mcp_connecting:
                model_label.update(
                    f"[yellow]MCP connecting…[/yellow]  {model_text}")
            else:
