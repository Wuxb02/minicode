...
from minicode.commands.registry import Command, CommandContext, CommandRegistry, CommandType
    ...
    for skill_name, skill_desc in loader.get_catalog():
        ...
        def make_handler(name: str) -> callable:
            ...
            async def handler(ctx: CommandContext) -> None:
                exe = ctx.config.get(
                    "skill_executor") if executor is None else executor
                if exe is None:
                    ctx.ui.add_system_message("Skill 执行器未初始化")
                    return

                skill_loader: SkillLoader | None = ctx.config.get(
                    "skill_loader")
